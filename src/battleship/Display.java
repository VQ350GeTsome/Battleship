...
public class Display extends javax.swing.JFrame {
    ...
    private float wave = -100, waveD = 0.20f;
    private int time = 0;

    private javax.swing.Timer backgroundTimer, gameTimer;
    ...
    private void initBackgroundTimer() {
        this.backgroundTimer = new javax.swing.Timer(20, (java.awt.event.ActionEvent e) -> {
            update();
            battleShip.displayBoards(wave, time);
        });
    }
    
    private void initGameTimer() {
        this.gameTimer = new javax.swing.Timer(100, (java.awt.event.ActionEvent e) -> {
            battleShip.promptAlgo();
            check();
        });
    }
    ...
    private void update() {
        time++;
        wave += waveD;
        if (-100 >= wave || wave >= 100) waveD *= -1;
    }
    
    private void check() {
        if (!BattleShip.hasGameStarted()) return;
        // Check if anyone has won.
        if (battleShip.playerBoardAreShipsSunk()) { 
        ...
        else if (battleShip.opponentBoardAreShipsSunk()) { 
            javax.swing.JOptionPane.showMessageDialog(
                null, 
                "You won!", 
                "Victory...", 
